...
{
	int numero;	
	 char  caracter; 	//la chaine a modifier
	cout << endl << "Donnez le numero de la chaine a modifier: ";
	cin.ignore(INT_MAX, '\n');
	cin >> numero;
	cout << endl << "Saisissez la nouvelle chaine ";

	tab[numero] = new char[size];	//reservation de memoire pour une chaine
	cin.getline(tab[numero], size_ch, '\t');	//saisie de la chaine
	cin.ignore(INT_MAX, '\n');
}
...
void trier(char* tab[], int const& nbre){
	...
	for (int k = 1 ; k  < nbre ; k++){

		char* temp =  tab[k];

		int j = k-1 ;
		int  result = strcmp(temp, tab[j]);

		while (j>=0 && (result ==0 || result < 0) ){
